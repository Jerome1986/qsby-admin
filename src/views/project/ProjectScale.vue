...
import type { ProjectScaleItem } from '@/types/Project'
import {
  deleteProjectScale,
  getProjectScales,
  projectAddScale,
...
const tableData = ref<ProjectScaleItem[]>([])
...
const dialogTitle = ref('新增合作规模')
const formData = ref<Partial<ProjectScaleItem>>({
  name: '',
  sort: 0,
  status: 'active',
})
...
const getList = async (pageNum: number, pageSize: number) => {
  loading.value = true
  try {
    const res = await getProjectScales(pageNum, pageSize)
...
const handleAdd = () => {
  dialogTitle.value = '新增合作规模'
  formData.value = { name: '', sort: 0, status: 'active' }
  dialogVisible.value = true
}

const handleEdit = (row: ProjectScaleItem) => {
  dialogTitle.value = '编辑合作规模'
  formData.value = { ...row }
  dialogVisible.value = true
}

const handleDelete = (row: ProjectScaleItem) => {
  ElMessageBox.confirm(`确认删除「${row.name}」吗？`, '提示', {
    ...
    .then(async () => {
      try {
        await deleteProjectScale(row._id)
...
<template>
  <div class="project-scale">
    <el-card shadow="never" class="toolbar-card">
      <el-button type="primary" :icon="Plus" @click="handleAdd">新增合作规模</el-button>
    </el-card>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="420px" destroy-on-close>
      <el-form :model="formData" label-width="80px">
        <el-form-item label="名称">
          <el-input v-model="formData.name" placeholder="如：10W以下、50W-100W" clearable />
        </el-form-item>
