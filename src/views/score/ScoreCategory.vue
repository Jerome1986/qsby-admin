...
import type { ScoreProductCategory } from '@/types/ScoreProduct'
import {
  deleteScoreCategory,
  getScoreCategories,
  scoreCategoryAdd,
...
/** 分类列表数据 */
const tableData = ref<ScoreProductCategory[]>([])
/** 列表加载状态 */
...
/** 表单数据 */
const formData = ref<Partial<ScoreProductCategory>>({
  name: '',
  sort: 0,
  status: 'active',
})
...
const getList = async (pageNum: number, pageSize: number) => {
  loading.value = true
  try {
    const res = await getScoreCategories(pageNum, pageSize)
...
/** 打开编辑弹窗 */
const handleEdit = (row: ScoreProductCategory) => {
...
/** 删除分类 */
const handleDelete = (row: ScoreProductCategory) => {
  ElMessageBox.confirm(`确认删除「${row.name}」吗？`, '提示', {
    ...
    .then(async () => {
      try {
        await deleteScoreCategory(row._id)
