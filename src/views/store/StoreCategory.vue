...
import type { StoreCategoryItem } from '@/types/Store'
import {
  deleteStoreCategory,
  getStoreCategories,
  storeAddCategory,
...
/** 分类列表数据 */
const tableData = ref<StoreCategoryItem[]>([])
/** 列表加载状态 */
...
/** 表单数据 */
const formData = ref<Partial<StoreCategoryItem>>({
  name: '',
  sort: 0,
  status: 'active',
})
...
const getList = async (pageNum: number, pageSize: number) => {
  loading.value = true
  try {
    const res = await getStoreCategories(pageNum, pageSize)
...
/** 打开编辑弹窗 */
const handleEdit = (row: StoreCategoryItem) => {
...
/** 删除分类 */
const handleDelete = (row: StoreCategoryItem) => {
  ElMessageBox.confirm(`确认删除「${row.name}」吗？`, '提示', {
    ...
    .then(async () => {
      try {
        await deleteStoreCategory(row._id)
